...
int main(){
	...
	rxcfg.fs_hz = MHZ(4);   // 2.5 MS/s rx sample rate
    ...
    if(!ctx){
        ...
        std::cout << "IIO context created successfully" << std::endl;
    }
...
// The cf-ad9361-dds-core-lpc is the DAC/TX output driver that controls the TX DMA and the TX HDL core, including the DDS.
    ...
    rxstream = iio_buffer_create_stream(rxbuf, 4, 1024*1024);
    txstream = iio_buffer_create_stream(txbuf, 4, 1024*1024);
